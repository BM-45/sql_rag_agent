...
# Re-ranker: cross-encoder for precise scoring
...
# SQL Node 1: Retrieve schema from ChromaDB + Re-rank with cross-encoder
...
def retrieve_schema(state: AgentState) -> dict:
    ...
    # Step 1: Retrieve top 10 from ChromaDB (fast, rough)
    ...
    print(f"[RETRIEVAL] ChromaDB returned {len(candidates)} candidates")

    # Step 2: Re-rank with cross-encoder (slow, precise)
    pairs = [[question, meta["original_context"]] for meta in candidates]
    rerank_scores = reranker.predict(pairs)

    # Combine and sort by re-ranker score (higher = more relevant)
    ranked = sorted(
        zip(rerank_scores, distances, candidates),
        key=lambda x: x[0],
        reverse=True
    )

    # Log before vs after re-ranking
    print(f"[RETRIEVAL] Before re-rank (vector order):")
    for i, meta in enumerate(candidates[:5]):
        print(f"  {i+1}. dist={distances[i]:.4f} | table={meta.get('table_names', 'unknown')}")

    print(f"[RETRIEVAL] After re-rank (cross-encoder order):")
    for i, (score, dist, meta) in enumerate(ranked[:5]):
        print(f"  {i+1}. rerank={score:.4f} | dist={dist:.4f} | table={meta.get('table_names', 'unknown')}")

    # Step 3: Take top 3 after re-ranking
    top_schemas = [meta["original_context"] for _, _, meta in ranked[:3]]

    return {"schema": "\n\n".join(top_schemas)}


# SQL Node 2: LLM generates SQL query
